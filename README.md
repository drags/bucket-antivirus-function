...
If you want to scan a subset of the bucket you can use the `--prefix` option to filter objects. Remember that S3 objects are not real paths and prefixes match on substrings, so `--prefix=test` will match `test`, `testfoo`, and `testing/foo`:
